...
def softmax_overflow_demo(large_value):
    ...
    """
    result = array_exp(np.array([large_value]))[0]
    return {
        'naive_exp': float(result),
        'overflowed': bool(np.isinf(result))
    }
...
def stable_softmax_2d_rowwise(logits):
    ...
    row_max = max_along_axis(logits, axis=1)[:, np.newaxis]
...
def stack_y_batch(data, offsets, block_size):
    ...
    return np.stack(
        [slice_y_at_offset(data, i, block_size) for i in offsets],
        axis=0
    )
...
def log_prob_of_pair(p_matrix, current_id, next_id):
    ...
    return float(array_log(np.array([probability]))[0])
...
def forward_logits_onehot(onehot, w_matrix):
    # Compute logits for the neural bigram model as onehot @ W.
...
def sample_from_neural_bigram(w, start_id, num_tokens, itos):
    ...
    for _ in range(num_tokens):
        ...
        probs = logits_to_probs_rowwise(logits)
        current_id = sample_next_token(probs, 0, rng)
...
def linear_forward(x, w):
    # Compute Y = X @ W and cache inputs for the backward pass.
    y = x @ w
    return {
        'y': y,
        'cache': {
...
def derive_linear_dw_on_paper():
    ...
    return (
        "Y = X @ W\n"
        "By the chain rule, the gradient with respect to W is formed by "
        "multiplying the transpose of X by the upstream gradient dY.\n"
        "Shapes: X (B, D_in), W (D_in, D_out), dY (B, D_out), "
        "so dL/dW has shape (D_in, D_out).\n"
        "dL/dW = X.T @ dY"
    )
...
def bias_add_backward_db(dy, cache):
    ...
    b_shape = cache['b_shape']
    return np.sum(dy, axis=0).reshape(b_shape)
...
def layernorm_forward_variance(x, mean):
    ...
    return sum_keepdims(squared, axis=-1) / x.shape[-1]
...
def layernorm_backward_subtract_mean(dy, cache):
    ...
    """
    x = cache['x']
    mean_dy = sum_keepdims(dy, axis=-1) / x.shape[-1]
    return dy - mean_dy
...
def layernorm_backward_implementation(d_out, cache):
    # Reuse the complete LayerNorm backward implementation.
...
def create_positional_embedding(block_size, d_model, scale=0.02):
    ...
    random_values = make_2d_random(block_size, d_model, seed=None)
...
def embedding_sum_backward(d_out):
    ...
    d_token_emb = d_out
    d_pos_emb = sum_axis0(d_out.reshape(d_out.shape[0], -1)).reshape(
        d_out.shape[1], d_out.shape[2]
    )
...
def create_qkv_projections(d_model, d_head, scale=0.02):
    # Create deterministic Q, K, V projection matrices using seeds 0, 1, and 2.
...
def output_projection_backward(d_proj, cache):
    ...
    dw_o = np.einsum('bth,btm->hm', attn_out, d_proj)
...
def qk_scores_backward(d_scores, cache):
    ...
    d_k = np.matmul(np.swapaxes(d_scores, -1, -2), q)
...
def qkv_projection_backward(d_q, d_k, d_v, cache):
    ...
    dx_q = np.matmul(d_q, w_q.T)
    dx_k = np.matmul(d_k, w_k.T)
    dx_v = np.matmul(d_v, w_v.T)

    dx = dx_q + dx_k + dx_v

    dw_q = np.einsum('bti,btj->ij', x, d_q)
    dw_k = np.einsum('bti,btj->ij', x, d_k)
    dw_v = np.einsum('bti,btj->ij', x, d_v)
...
def get_multihead_n_heads(config):
    # Return the number of attention heads stored in the multi-head config dict.
...
def multihead_masked_softmax_scores(scores, mask):
    ...
    flat_scores = masked_scores.reshape(-1, T)
...
def transpose_heads_to_back(x_heads):
    """Move the heads axis back to shape (B, T, n_heads, d_head)."""
...
def transformer_block_forward(x, block_params):
    ...
    def attention_sublayer_forward(x_in, attn_params):
        Wq = attn_params['Wq']
        Wk = attn_params['Wk']
        Wv = attn_params['Wv']
        Wo = attn_params['Wo']
        bo = attn_params.get('bo', np.zeros(x_in.shape[-1]))

        d_model = x_in.shape[-1]
        n_heads = attn_params.get('n_heads', 1)
        d_head = attn_params.get('d_head', d_model // n_heads)
        ...
        mask = build_causal_mask(x_in.shape[1])
        weights = multihead_masked_softmax_scores(scores, mask)

        attn_heads = multihead_weighted_sum(weights, v_heads)
        attn_back = transpose_heads_to_back(attn_heads)
        merged = merge_heads_to_d_model(attn_back)

        out = np.matmul(merged, Wo) + bo
    ...
    def ffn_sublayer_forward(x_in, ffn_params):
        ...
        a1, relu_cache = ffn_activation_forward(first['h1'])
        ...
        return {
            ...
            'cache': {
                ...
                'linear1_cache': first['cache'],
                'linear2_cache': second['cache']
            }
        }
    ...
    # FFN branch receives the output of the attention residual.
...
def transformer_block_backward(d_y, cache, block_params):
    ...
    """
    # Rebuild a complete cache so all backward helpers have the fields they need.
    x = cache['attn_branch']['x']
    full_cache = _complete_block_cache(x, block_params)

    # ---------------------------------------------------------
    # Backward through the second branch:
    # y = h1 + FFN(LN2(h1))
    # ---------------------------------------------------------
    ...
    d_h1_ln2, d_gamma2, d_beta2 = layernorm_backward_affine(
        d_ln2_out,
        ffn_branch['ln_cache']
    )

    d_h1 = d_h1_skip + d_h1_ln2

    # ---------------------------------------------------------
    # Backward through the first branch:
    # h1 = x + Attn(LN1(x))
    # ---------------------------------------------------------
    ...
    d_x_ln1, d_gamma1, d_beta1 = layernorm_backward_affine(
        d_ln1_out,
        attn_branch['ln_cache']
    )

    d_x = d_x_skip + d_x_ln1
...
def stack_transformer_blocks(n_layers, d_model, n_heads, d_ff):
    ...
    """
    # Validate the multi-head configuration.
    d_head = compute_d_head(d_model, n_heads)

    blocks = []

    for _ in range(n_layers):
        attn_proj = create_multihead_qkv_projections(d_model, scale=0.02)
        Wo = create_multihead_output_projection(d_model, scale=0.02)
...
def lm_head_linear_forward(x, w_lm, b_lm):
    ...
    bias_result = bias_add_forward(linear_result['y'], b_lm)
...
def full_model_backward(d_logits, caches, model_params):
    ...
    # ---------------------------------------------------------
    # 1. Backward through the LM head
    # logits = x @ w_lm + b_lm
    # ---------------------------------------------------------
    ...
    x_lm_flat = x_lm.reshape(-1, D)
    d_logits_flat = d_logits.reshape(-1, V)

    dw_lm = x_lm_flat.T @ d_logits_flat
    db_lm = np.sum(d_logits_flat, axis=0)
    d_ln_f_flat = d_logits_flat @ w_lm.T
    d_ln_f = d_ln_f_flat.reshape(B, T, D)

    # ---------------------------------------------------------
    # 2. Backward through final LayerNorm
    # Flatten B,T into one dimension so the existing
    # layernorm_backward_full helper can be reused.
    # ---------------------------------------------------------
    ln_cache = caches['ln_f']

    ln_x = ln_cache['x']
    ln_x_hat = ln_cache['x_hat']
    ln_mean = ln_cache['mean']
    ln_var = ln_cache['var']
    ln_gamma = ln_cache['gamma']

    ln_x_flat = ln_x.reshape(-1, D)
    ln_x_hat_flat = ln_x_hat.reshape(-1, D)
    ln_mean_flat = ln_mean.reshape(-1, 1)
    ln_var_flat = ln_var.reshape(-1, 1)
    ...
    d_blocks_in = ln_grads['dx'].reshape(B, T, D)
    d_gamma_f = ln_grads['dgamma']
    d_beta_f = ln_grads['dbeta']

    # ---------------------------------------------------------
    # 3. Backward through the Transformer blocks
    # ---------------------------------------------------------
    ...
    # ---------------------------------------------------------
    # 4. Backward through token + positional embedding sum
    # ---------------------------------------------------------
    token_ids = caches['emb']['tok_cache']['token_ids']

    d_token_emb = np.zeros_like(
        model_params['tok_emb'],
        dtype=np.float64
    )
    ...
    seq_len = caches['emb'].get(
        'seq_len',
        token_ids.shape[1]
    )

    d_pos_emb = np.zeros_like(
        model_params['pos_emb'],
        dtype=np.float64
    )
    ...
    # ---------------------------------------------------------
    # 5. Assemble gradient tree matching model_params
    # ---------------------------------------------------------
    return {
        'tok_emb': d_token_emb,
        'pos_emb': d_pos_emb,
...
def top_k_filter(logits, k):
    ...
    if k <= 0:
        return np.full_like(logits, -np.inf, dtype=np.float64)

    indices = np.argpartition(logits, -k, axis=1)[:, -k:]

    filtered = np.full_like(logits, -np.inf, dtype=np.float64)
...
def generation_loop_for_n_steps(
    ...
    for _ in range(n_new_tokens):
        cropped_context = crop_context_to_block_size(
            context_ids,
            block_size
        )
        ...
        temperature_logits = apply_temperature(
            last_logits,
            temperature
        )
